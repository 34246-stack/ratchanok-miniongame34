...
let gameOver = false;
let gameStarted = false; // บล็อกสถานะเริ่มเกม
let score =0;
let bananasCollected = 0; // ระบบเก็บกล้วยแยก
let time =0;
...
function spawnRandomObject() {
    ...
    let isBanana = Math.random() <= 0.35; // ปรับโอกาสเจอกล้วยเป็น 35%
    ...
    // ถ้าเป็นกล้วยชุดใหญ่ ให้เว้นระยะเวลาสุ่มรอบถัดไปนานขึ้นหน่อย (1800ms - 3000ms) กล่องจะได้ไม่โผล่มาทับกล้วย
...
function createBanana() {
    if (gameOver) return;
    // สุ่มจำนวนกล้วยที่จะออกมาในกลุ่มนั้น (เช่น สุ่มให้税ออกมาทีละ 3 ถึง 5 อันติดกัน)
    let bananaGroupSize = Math.floor(Math.random() * (5 - 3 + 1)) + 3; 
    
    // กำหนดระยะห่างระหว่างกล้วยแต่ละลูกในแถว (60 พิกเซล กำลังสวย ไม่ชิดหรือไม่ห่างเกินไป)
    let spacing = 60; 

    for (let i = 0; i < bananaGroupSize; i++) {
        let banana = {
            img: bananaImg,
            x: boxX + (i * spacing), // กล้วยลูกถัดไปจะขยับไปทางขวา เรียงต่อกันเป็นแถว
...
function update() {
    ...
    for (let i = 0; i < bananaArray.length; i++) {
        ...
        if (onCollision(player, banana)) {
            bananasCollected += 1; 
            score += 500; // คะแนนพิเศษจากการเก็บกล้วย
    ...
    context.beginPath();
    context.roundRect(10, 10, 180, 85, 8); // ขยายป้ายให้คลุมข้อมูล 3 บรรทัด
    context.fill();
